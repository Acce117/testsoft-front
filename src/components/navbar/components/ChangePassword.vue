<template>
    ...
    <Dialog v-model:visible="visible" @after-hide="cleanForm" modal :header="$t('change_password.title')"
...
const { mutate: changePassword, isPending: isAddPending } = useMutation({
    ...
    onSuccess: (data) => {
        ...
        if (data.status == 401) {
            toast.add({ severity: 'error', summary: t('table.something_wrong'), detail: t('change_password.wrong_password'), life: 5000 });
        } else {
