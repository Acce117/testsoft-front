<template>
<div relative flex flex-col border-1 text-slate-800 border-slate-200 border-solid bg-white shadow-xl rounded-xl w-21rem text-black pa-1rem gap-1rem anim-scale-0.5>
        <img
          rd-4rem
          ...
          v-if="props.img"
        />
        <div v-else rounded-full size-15 flex justify-center border-4 border-solid border-slate-800  overflow-hidden>
            <i class="pi pi-user"  style="font-size: 3rem"></i>
        </div>

        <span font-bold text-md 
          >{{ props.name }}</span
        >
