<template>
    <FormDialog :model :submit-function :success-function ref="dialog">
        <template #form>
            ...
            <VYesNoQuestion v-if="testBuilder.getTest().isPsicometricTest()" v-model="model.is_correct"
                label="¿Se puede considerar correcta esta respuesta?" name="is_correct"
                :default="model.correct_answer.id_correct_answer !== undefined" />
            ...
            <VInput v-if="!isQuestionTypeMultipleValueSettled" v-model="model.tribute.tribute_value" number name="tribute_value"
...
const isQuestionTypeMultipleValueSettled = ref(false)
...
const show = (type?:string|number) => {
    if(type==5){
        isQuestionTypeMultipleValueSettled.value = true
        model.value.tribute.tribute_value = 0
    }
